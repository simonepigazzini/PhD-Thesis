...
\begin{figure}[ht!]
  \centering
  \includegraphics[width = 0.45\textwidth]{figures/upgrade/roc_iso_comparison_BTL.pdf}
  \includegraphics[width = 0.45\textwidth]{figures/upgrade/roc_iso_comparison_ETL.pdf}  \
...
\begin{figure}[h!]
  \centering
  \includegraphics[width = .7\textwidth]{figures/upgrade/roc_iso_and_ratios_BTL.pdf}
  \includegraphics[width = .7\textwidth]{figures/upgrade/roc_iso_and_ratios_ETL.pdf}
...
\begin{figure}
  \centering
  \includegraphics[width = .7\textwidth]{figures/upgrade/HZZRapidity_TimingStudy_200PU.pdf}
...
\begin{figure}[h!]
  \centering
  \includegraphics[width = 0.6\textwidth]{figures/upgrade/vtxeff.pdf}
...
According to simulation, at 140 pileup events, the efficiency drops below $40%$ for \Hgg events
produced through gluon fusion (the main contribution to the total Higgs boson cross-section),
and it degrades to about $30%$ at 200 pileup events.
...
% The procedure is qualitatively illustrated in Figure~\ref{fig:dipho_triang},
The vertex position
...
\begin{figure}[h!]
  \centering
  \includegraphics[width = 0.45\textwidth]{figures/upgrade/vtxResolLargeDetaCaloOnly.pdf}
  \includegraphics[width = 0.45\textwidth]{figures/upgrade/vtxResolSmallDetaCaloOnly_CrossOnly.pdf}
...
\begin{figure}[htb!]
\centering
\includegraphics[width=0.48\linewidth]{figures/upgrade/chi2.pdf}
\includegraphics[width=0.48\linewidth]{figures/upgrade/Rank.pdf}
...
% The gain in mass resolution has been projected onto  
% the \Hgg fiducial cross section measurement at HL-LHC (Figure~\ref{hgg_projection}.
% The projection neglects the increased acceptance provided by the use of timing
% in the photon isolation which is expected to provide a similar performace gain as
% for muons. The results of the current \Hgg analysis are projected for an
% integrated luminosity of 3000\fbinv. 

% is estimated in the context of projections of  the most recent Run 2
% results~\cite{CMS:2017cwx}.  
 
% The impact on the invariant mass resolution and on the
% signal-to-background ratio is estimated for different CMS upgrade
% scenarios.
% At 140 pileup without timing information, the primary vertex selection
% efficiency is reduced to 40\% (``S2+ no timing''). 
% With precision timing available only for the photons from the
% calorimeters, the vertex is correctly identified for a fraction of the
% events via triangulation, corresponding to the sample
% with $|\Delta\eta|>0.8$, and the total vertex selection efficiency is
% estimated to be about 55\% (``S2+Calorimeter timing''). 
% With precision timing available both for the photons in the
% calorimeters as well as for the charged particles in the event,  a
% fivefold reduction in the effective pileup is assumed for all of the
% events, and the total primary vertex selection efficiency is increased
% to 75\% (``S2+Calorimeter and MTD timing''), nearly fully recovering
% the present Run 2 performance (``S2''). 

% % In this case, four scenarios are compared.  In the first scenario
% %``S2'', no degradation due to higher pileup is applied, and therefore
% %the present Run 2 performance of $\sim 80$\% primary vertex selection
% %efficiency is retained.  
% %In the ``S2+ Pessimistic'' scenario, a degradation of performance is
% %applied corresponding to 140~PU conditions, such that the primary
% %vertex selection efficiency is reduced to 40\%.  
% %There is also a 5\% reduction in efficiency affecting both the signal and
% %irreducible background, in order to account for degraded photon
% %identification performance in the high pileup conditions.  Additional
% %variations of the S2+ scenario are also considered.  The ``S2+
% %intermediate'' scenario corresponds to the case where precision timing
% %is available for the photons from the calorimeters, and therefore a fivefold
% %reduction in the effective pileup is assumed for 50\% of the events,
% %corresponding to the sample with $|\Delta\eta|>0.8$, and the total
% %vertex selection efficiency is therefore increased back to 55\%.  In
% %the final ``S2+ Optimistic'' scenario, precision timing is assumed to
% %be available both for the photons in the calorimeters, as well as for
% %the charged particles in the event.  In this case a fivefold reduction in
% %the effective pileup is assumed for all of the events, and the total
% %primary vertex selection efficiency is increased to 75\%, nearly fully
% %recovering the present Run 2 performance.  
% The signal lineshapes %and projected uncertainty on the fiducial cross
% %section measurement 
% for the four scenarios are shown in Fig.~\ref{fig:hggprojection}.  The
% improvement from precision timing with the full upgrade scope 
% corresponds to around 15\% reduction in the statistical uncertainty of
% the fiducial cross section measurement. 
% Although there are still significant systematic uncertainties on this
% measurement at 3000~fb$^{-1}$, the statistical-only uncertainty can
% serve as a proxy for finely binned differential cross sections and/or
% differential cross sections measured as a ratio to the fiducial cross
% section, for which most systematic uncertainties will cancel.  
% For such statistically limited measurements, the improvements from
% precision timing correspond to an approximately 30\% increase in
