...
\providecommand{\refChIso}{\ensuremath{Ch_{iso}/p_T^{muon}}}
...
The vertex line density distributions for the current LHC beamspont and
...
The vertex mergin occuring with $\Delta z$-based association criterion described in the previous section
...
the largest fraction of hadronic activity in $p-p$ collisions and therefore are the most important contrinution
...
muon to a true generetor-level muon
Non-prompt muons from semileptonic decays of heavy flavour hadrons in $t\bar{t}$ events are
considered rappresentative of non-isolated, background muons. Muons coming from the decay of the W boson produced
by the top quark decay are rejected based on generetor-level information and only muons matching a generator-level
...
\begin{itemize}
  ...
  \caption{Projections for yield enhancement in $H\toZZ\to4\mu$ as a function of the Higgs boson rapidity.
...
is subordinated to the precision on two measurment: the energy resolution of the ECAL and the precision on the
angle between the two photons related to the indentification of the diphoton vertex.
...
The HL-LHC era will prived an unprecedented amount of data to the ATLAS and CMS experiments through a record
...
The inclusion of the time imformation in the event reconstruction has been proved, through simulation studies,
to provide a unique way to mitigate the deterioration of performance due to pile-up and also open the possibility
to measure observables otherwise inaccessible (mass of long lived SUSY particles). The same simulation
...
ECAL barrel sensors and also those proposed for the implementation of the MTD (LYSO crystal with SiPM photodetectors for
